...
const char* data_type_string(TensorElementType data_type) {
    switch (data_type) {
        ...
        case DATA_TYPE_UINT4:
            return "DATA_TYPE_UINT4";
        case DATA_TYPE_INT4:
            return "DATA_TYPE_INT4";
        case DATA_TYPE_FLOAT4E2M1:
            return "DATA_TYPE_FLOAT4E2M1";
        case DATA_TYPE_FLOAT8E8M0:
            return "DATA_TYPE_FLOAT8E8M0";
        case DATA_TYPE_UINT2:
            return "DATA_TYPE_UINT2";
        case DATA_TYPE_INT2:
            return "DATA_TYPE_INT2";
        default:
            return "DATA_TYPE_UNDEFINED";
    }
}
...
size_t compute_data_size(TensorElementType data_type, int rank, const int* shape) {
    if (rank < 0 || (rank > 0 && shape == NULL)) return 0;
    ...
    switch (data_type) {
        ...
        case DATA_TYPE_UINT4:
        case DATA_TYPE_FLOAT4E2M1:
            return (num_elements + 1) / 2;  // 2 per byte
        case DATA_TYPE_INT2:
        case DATA_TYPE_UINT2:
            return (num_elements + 3) / 4;  // 4 per byte
        case DATA_TYPE_UNDEFINED:
        case DATA_TYPE_STRING:
            return 0;
        default: {
...
void tensors_set(Tensors* tensors, int index, const char* name, TensorElementType data_type, int rank, const int* shape,
    ...
    if (tensors == NULL || index < 0 || index >= tensors->num_tensors) return;
    ...
    if (rank > 0 && shape != NULL) {
        ...
        if (d->shape != NULL) memcpy(d->shape, shape, (size_t)rank * sizeof(int));
    } else {
        d->shape = NULL;
    }
}
...
const TensorDescriptor* tensors_find(const Tensors* tensors, const char* name) {
    if (tensors == NULL || name == NULL) return NULL;
    for (int i = 0; i < tensors->num_tensors; i++) {
        if (tensors->tensors[i].name != NULL && strcmp(tensors->tensors[i].name, name) == 0)
...
bool tensors_validate(const Tensors* tensors) {
    ...
    for (int i = 0; i < tensors->num_tensors; i++) {
        ...
        if (d->name == NULL || d->name[0] == '\0') printf("tensors_validate: WARNING descriptor %d has no name\n", i);
        ...
        if (d->data_type != DATA_TYPE_UNDEFINED && d->rank >= 0 && d->shape != NULL) {
            ...
            if (expected > 0 && d->data_size != expected)
                printf(
                    "tensors_validate: WARNING descriptor %d data_size is %zu but expected %zu"
    ...
    for (int i = 0; i < tensors->num_tensors; i++) {
        if (tensors->tensors[i].name == NULL) continue;
        for (int j = i + 1; j < tensors->num_tensors; j++) {
            if (tensors->tensors[j].name != NULL && strcmp(tensors->tensors[i].name, tensors->tensors[j].name) == 0)
...
bool tensors_compare(const Tensors* a, const Tensors* b) {
    ...
    if (a->id != b->id || a->num_tensors != b->num_tensors) return false;

    for (int i = 0; i < a->num_tensors; i++) {
        const TensorDescriptor* ta = &a->tensors[i];
        const TensorDescriptor* tb = &b->tensors[i];

        if (ta->data_type != tb->data_type) return false;
        if (ta->rank != tb->rank) return false;
        if (ta->data_size != tb->data_size) return false;

        // compare names
        if (ta->name != tb->name) {
            if (ta->name == NULL || tb->name == NULL) return false;
            if (strcmp(ta->name, tb->name) != 0) return false;
        }

        // compare shapes
        for (int j = 0; j < ta->rank; j++) {
            if (ta->shape[j] != tb->shape[j]) return false;
        }

        // compare data
        if (ta->data_size > 0) {
            if (ta->data == NULL || tb->data == NULL) return false;
            if (memcmp(ta->data, tb->data, ta->data_size) != 0) return false;
        }
    }
...
void tensors_print(const Tensors* tensors) {
    ...
    for (int i = 0; i < tensors->num_tensors; i++) {
        const TensorDescriptor* d = &tensors->tensors[i];
        printf("  [%d] name='%s'  type=%s  rank=%d  data_size=%zu  shape=[", i, d->name ? d->name : "(null)",
               data_type_string(d->data_type), d->rank, d->data_size);
        for (int j = 0; j < d->rank; j++) {
            if (j > 0) printf(", ");
...
Config* config_alloc(void) { return (Config*)calloc(1, sizeof(Config)); }
...
void config_print(const Config* c) {
    ...
    for (int i = 0; i < c->length; i++) printf("  [%d] '%s' = '%s'\n", i, c->keys[i], c->values[i]);
}
...
const char* runtime_status_string(RuntimeStatus status) {
    switch (status) {
        case RUNTIME_STATUS_SUCCESS:
            return "RUNTIME_STATUS_SUCCESS";
        case RUNTIME_STATUS_ERROR:
            return "RUNTIME_STATUS_ERROR";
        case RUNTIME_STATUS_NOT_INITIALIZED:
            return "RUNTIME_STATUS_NOT_INITIALIZED";
        case RUNTIME_STATUS_ALREADY_INITIALIZED:
            return "RUNTIME_STATUS_ALREADY_INITIALIZED";
        case RUNTIME_STATUS_MODEL_NOT_LOADED:
            return "RUNTIME_STATUS_MODEL_NOT_LOADED";
        case RUNTIME_STATUS_INVALID_ARGUMENT:
            return "RUNTIME_STATUS_INVALID_ARGUMENT";
        case RUNTIME_STATUS_INVALID_MODEL:
            return "RUNTIME_STATUS_INVALID_MODEL";
        case RUNTIME_STATUS_FILE_NOT_FOUND:
            return "RUNTIME_STATUS_FILE_NOT_FOUND";
        case RUNTIME_STATUS_OUT_OF_MEMORY:
            return "RUNTIME_STATUS_OUT_OF_MEMORY";
        case RUNTIME_STATUS_INVALID_TENSOR:
            return "RUNTIME_STATUS_INVALID_TENSOR";
        case RUNTIME_STATUS_TENSOR_SHAPE_MISMATCH:
            return "RUNTIME_STATUS_TENSOR_SHAPE_MISMATCH";
        case RUNTIME_STATUS_TENSOR_TYPE_MISMATCH:
            return "RUNTIME_STATUS_TENSOR_TYPE_MISMATCH";
        case RUNTIME_STATUS_NO_OUTPUT_AVAILABLE:
            return "RUNTIME_STATUS_NO_OUTPUT_AVAILABLE";
        case RUNTIME_STATUS_INFERENCE_ERROR:
            return "RUNTIME_STATUS_INFERENCE_ERROR";
        case RUNTIME_STATUS_NOT_IMPLEMENTED:
            return "RUNTIME_STATUS_NOT_IMPLEMENTED";
        case RUNTIME_STATUS_TIMEOUT:
            return "RUNTIME_STATUS_TIMEOUT";
        case RUNTIME_STATUS_DEVICE_ERROR:
            return "RUNTIME_STATUS_DEVICE_ERROR";
        case RUNTIME_STATUS_UNKNOWN_ERROR:
            return "RUNTIME_STATUS_UNKNOWN_ERROR";
        case RUNTIME_STATUS_INVALID_MODEL_ID:
