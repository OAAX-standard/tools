...
static void get_architecture(char *buf, size_t size) {
    ...
    switch (sysInfo.wProcessorArchitecture) {
        case PROCESSOR_ARCHITECTURE_AMD64:
            strncpy(buf, "x64", size);
            break;
        case PROCESSOR_ARCHITECTURE_INTEL:
            strncpy(buf, "x86", size);
            break;
        case PROCESSOR_ARCHITECTURE_ARM64:
            strncpy(buf, "ARM64", size);
            break;
        default:
            strncpy(buf, "Unknown", size);
    }
...
void get_system_info(SystemInfo *info) {
    ...
    detect_instruction_sets(info->instruction_sets, sizeof(info->instruction_sets));

    MEMORYSTATUSEX memStatus;
    memStatus.dwLength = sizeof(memStatus);
    if (GlobalMemoryStatusEx(&memStatus)) {
        info->total_ram_mb = (unsigned long long)(memStatus.ullTotalPhys / (1024 * 1024));
    }
    ...
    if (GlobalMemoryStatusEx(&mem)) {
        ...
        info->ram_usage_percent = (double)info->used_ram_mb / info->total_ram_mb * 100.0;
    }
}
