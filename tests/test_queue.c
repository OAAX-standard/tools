...
static DWORD WINAPI consumer_thread(LPVOID arg) {
  ...
  while (received < ta->count) {
    ...
    if (50 - received > 0) sleep_ms(50 - received);
  }
  return 0;
}
#else
...
static void* consumer_thread(void* arg) {
  ...
  while (received < ta->count) {
    ...
    if (10 - received > 0) sleep_ms(10 - received);
  }
  return NULL;
}
#endif
...
#ifdef _WIN32
  HANDLE prod = CreateThread(NULL, 0, producer_thread, &arg, 0, NULL);
  HANDLE cons = CreateThread(NULL, 0, consumer_thread, &arg, 0, NULL);
